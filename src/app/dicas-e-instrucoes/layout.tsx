  ...
  return (
    <div className="pb-20">
      <section className="mx-auto max-w-3xl px-6 pt-20 text-center">
        <span className="font-sans text-xs uppercase tracking-widest text-rose">Para os convidados</span>
      ...
      <nav className="mx-auto mt-10 flex max-w-3xl justify-center gap-8 border-b border-line px-6">
        {TABS.map((tab) => (
          <Link
            key={tab.href}
            href={tab.href}
            className="pb-4 font-sans text-sm uppercase tracking-widest text-ink-soft hover:text-rose"
          >
            {tab.label}
          </Link>
        ))}
      ...
      <div className="mx-auto max-w-3xl px-6 pt-12">{children}</div>
    </div>
  );
}
